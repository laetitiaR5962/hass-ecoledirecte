...
class EDApiClient:
    ...
    async def get_periodes_moyennes(
        ...
        """Récupère l'ensemble des périodes avec leurs moyennes générales et par discipline."""
        if FAKE_ON:
            json_resp = await load_json_file(
                self.test_folder + f"{eleve.eleve_id}_get_grades_evaluations.json"
            )
        ...
        if "data" not in json_resp or "periodes" not in json_resp["data"]:
            LOGGER.warning("get_periodes_moyennes: données introuvables [%s]", json_resp)
            return []
        ...
        for periode_json in periodes:
            ...
            # Extraire les moyennes générales de la période
            ensemble = periode_json.get("ensembleMatieres", {})
            if ensemble:
                periode_info["moyenne_generale"] = {
                ...
                # Extraire les matières / disciplines de la période
                """for disc in ensemble.get("disciplines", []):
                    periode_info["disciplines"].append({
                        "id": disc.get("id"),
                        "codeMatiere": disc.get("codeMatiere"),
                        "discipline": disc.get("discipline"),
                        "moyenne": (disc.get("moyenne") or "").replace(",", "."),
                        "moyenneClasse": (disc.get("moyenneClasse") or "").replace(",", "."),
                        "moyenneMin": (disc.get("moyenneMin") or "").replace(",", "."),
                        "moyenneMax": (disc.get("moyenneMax") or "").replace(",", "."),
                        "coef": disc.get("coef"),
                        "rang": disc.get("rang"),
                    })"""
